...
async def get_observations(
    ...
    param_idx = 1
    if student_id:
        clauses.append(f"o.user_id = ${param_idx}")
        params.append(student_id)
        param_idx += 1
    if experiment_id:
        clauses.append(f"o.experiment_id = ${param_idx}")
...
async def get_alerts(current_user=Depends(require_teacher), db=Depends(get_db)):
    ...
    inactive = await db.fetch(
        "SELECT u.name, u.id FROM users u WHERE u.role='student' AND u.id NOT IN (SELECT DISTINCT user_id FROM experiment_sessions WHERE started_at >= NOW() - INTERVAL '5 days')"
    )
    for s in inactive:
