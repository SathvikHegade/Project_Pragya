...
async def get_progress(current_user=Depends(get_current_user), db=Depends(get_db)):
    rows = await db.fetch(
        "SELECT experiment_id, MAX(score) as best_score, COUNT(*) as attempts, MAX(completed) as completed FROM experiment_sessions WHERE user_id=$1 GROUP BY experiment_id",
        current_user["id"]
    )
...
async def update_profile(data: dict, current_user=Depends(get_current_user), db=Depends(get_db)):
    ...
    set_parts = []
    params = []
    for idx, k in enumerate(updates, 1):
